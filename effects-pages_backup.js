function pageTurnForward(){
	...
	if(leftPage > lastPage){
		...
		//ANIMATION
		$('.right-page').css('float', 'left').animate({
				'width' : '0%',
				'padding-left' : '0px',
				'padding-right' : '0px'
			}, 500).animate({
		...
		$.get('/storybooks/dynamic/readBook.php?book=' + book_id + '&page=' + leftPage, function(data){
			$('.left-page').delay(1000).html(data);
			$('.left-page').delay(1000).css('background-image', 'url(/storybooks/img/pages/book-2-page-' + leftPage + '.png)');
		});
		$.get('/storybooks/dynamic/readBook.php?book=' + book_id + '&page=' + rightPage, function(data){
			$('.right-page').delay(1000).html(data);
			$('.right-page').delay(1000).css('background-image', 'url(/storybooks/img/pages/book-2-page-' + rightPage + '.png)');
		});
		//MAKE PAGE STAND OUT
		focusLeft();
		//CLEAR ACCESSORIES
